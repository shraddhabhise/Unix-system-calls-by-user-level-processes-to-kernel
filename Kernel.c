...
-- method to make waiting threads for acquiring resources in waiting thread list of condition variable
      ...
      method Signal (mutex: ptr to HoareMutex)
	   ...
	   mutex.GiveLock(t)									-- gives lock to the thread at the head of waiting list of condition variable
           ...
           readyList.AddToEnd (t)								-- adds this newly awekened thread with lock at the add of ready list
...
-- This behaviour is created to manage resource (thread) allocation to the requesting threads
-- It grants thread to the requesting thread and on completion of execution returns thread resource to freeList
-- When resource is not available it makes the requesting threads sleep on condition variable
-- when resource becomes availabe it signal waiting thread and let it grab the resource
	...
	threadTable=new array of Thread {MAX_NUMBER_OF_PROCESSES of new Thread}        -- creates array to hold all requesting threads
        freeList=new List [Thread]							-- creates new freelist for free resources to sit on
	aThreadBecameFree= new Condition						-- instantiate condition class
	threadManagerLock= new Mutex							-- instantiate mutex class
	aThreadBecameFree.Init()							 -- initializes condition variable
        threadManagerLock.Init()							-- initializes mutex lock for ThreadManager
	
	/* 
	threadTable[0].Init("Thread A")
	 threadTable[1].Init("Thread B")
	 threadTable[2].Init("Thread C")
	 threadTable[3].Init("Thread D")
	 threadTable[4].Init("Thread E")
	 threadTable[5].Init("Thread F")
	 threadTable[6].Init("Thread G")
	 threadTable[7].Init("Thread H")
	 threadTable[8].Init("Thread I")
	 threadTable[9].Init("Thread J")
	*/
	
	for j= 0 to MAX_NUMBER_OF_PROCESSES-1
		threadTable[j].Init("Thread:")
	...
	threadManagerLock.Lock()					 -- Locks the monitors mutex for thread manager

        if (freeList.IsEmpty())						 -- Checks if freeList is empty
             while( ! threadInUse )					 -- checks if threadInUse is empty

                threadInUse = freeList.Remove()				 -- if yes then removes process from freelist

                if (! threadInUse )
                  aThreadBecameFree.Wait(&threadManagerLock)		  -- if removed thread is empty then make a thread wait on condition variable aThreadBecameFree
               endIf
              endWhile
            else
              threadInUse = freeList.Remove()				 -- if freeList is not empty remove a thread to use it as next thread

            endIf
  
          threadInUse.status = JUST_CREATED				-- make thread status as just created
  
          threadManagerLock.Unlock()					-- Unlocks the monitor mutex for process Manager

          return threadInUse						  -- returns the process that is currently in use
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- This method is passed a ptr to a Thread;  It moves this thread
        -- to the FREE list.
         
	threadManagerLock.Lock()					  -- Locks the monitors mutex for process manager
	  th.status=UNUSED						  -- makes the status of thread as unused
	  freeList.AddToEnd(th)						  -- adds the free thread to freelist
          aThreadBecameFree.Signal(&threadManagerLock)			   -- signals the waiting thread for resources
...
-- This behaviour is created to manage resource (process control block) allocation to the requesting threads
-- It grants process Control block to the requesting thread and on completion of execution returns process control block resource to freeList
-- When process Control block is not available it makes the requesting threads sleep on condition variable
-- when resource becomes availabe it signal waiting thread and let it grab the resource
	...
	processManagerLock=new Mutex										-- creates object of mutex for process manager
	processManagerLock.Init()										-- initializes mutex for process manager
	aProcessBecameFree=new Condition									-- creates object Condition class 
	aProcessBecameFree.Init()										-- Initializes object 
	freeList=new List[ProcessControlBlock]									-- cretes new freelist to hold free resources
	processTable=new array of ProcessControlBlock{MAX_NUMBER_OF_PROCESSES of new ProcessControlBlock }      -- creates array to store requesting processes for pcb

	for i=0 to MAX_NUMBER_OF_PROCESSES-1
		processTable[i].Init()
		processTable[i].status=FREE									-- makes status of all process control block resources free
		freeList.AddToEnd(&processTable[i])								-- add all resources to processTable
	endFor
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        --
        -- This method provides  a new ProcessControlBlock; it will wait
        -- until one is available.
        -- It will also assign a new pid to the process that is in use

     var
        processInUse:ptr to  ProcessControlBlock
	
       processManagerLock.Lock()						-- Locks the monitors mutex for process manager

          if (freeList.IsEmpty())						-- Checks if freeList is empty
             while( ! processInUse )						-- checks if processInUse is empty
                processInUse = freeList.Remove()				-- if yes then removes process control block resource from freelist
                if (! processInUse )						-- if removed resource is empty then make a thread wait on condition variable aProcessBecameFree
                  aProcessBecameFree.Wait(&processManagerLock)
               endIf
              endWhile
            else
             processInUse = freeList.Remove()					-- if freeList is not empty remove a thread(process) to use it as next thread
            endIf
	  
	nextPid= nextPid + 1                                               
	processInUse.pid=nextPid						-- Here we have incremented the pid of process as it is a new process and is not free
        processInUse.status = ACTIVE						-- Set the status of process as Active

        processManagerLock.Unlock()						-- Unlocks the monitor mutex for process Manager

        return processInUse							-- returns the process that is currently in use
          
        endMethod

      ----------  ProcessManager . FreeProcess  ----------

-- This function returns a process control block after it is released by a process

      method FreeProcess (p: ptr to ProcessControlBlock)
        --
        -- This method is passed a ptr to a Process;  It moves this thread
        -- to the FREE list.
	-- and signals other threads waiting for processControlBlock

        processManagerLock.Lock()						-- Locks the monitors mutex for process manager
	p.pid= -1								-- make pid of process -1 to indicate that it s free now
        p.status=FREE								-- set the status of process as free
        freeList.AddToEnd(p)
        aProcessBecameFree.Signal(&processManagerLock)				-- signals the waiting thread for resources
        processManagerLock.Unlock()						 -- Unlocks the monitor mutex for process Manager
        endMethod
...
-- This method gets a new thread, 
-- initializes it 
--  calls startuserprocess method through fork 
...
-- This function will let a  new thread execute the StartUserProcess function, 
...
--step 3 Creating logical address space and loads executable file into it 

	initPC=pOpenFile.LoadExecutable(&pNewPCB.addrSpace)

--step 4 closes the executable file

	fileManager.Close(pOpenFile)

--step 5  compute the initial value of user level stack 

	initUserStackTop=(pNewPCB.addrSpace.numberOfPages* PAGE_SIZE)

--step 6 initializes the System stack top

	initSystemStackTop= & currentThread.systemStack[SYSTEM_STACK_SIZE-1]

--step 7 disables interrupts and turns a threads to user level thread
...
-- This behaviour is created to manage resource (frames) allocation to the requesting threads
-- It grants frames to the requesting thread and on completion of execution returns all frame resource to freeList
-- When frame is not available it makes the requesting threads sleep on condition variable
-- when resource becomes availabe it broadcasts all  waiting thread and let it grab the resource
...
-- the one and only "frameManager" object.  
	  ...
	  numWaitingGroups=0
          frameManagerLock = new Mutex						 -- instantiates lock for frame maanger
          frameManagerLock.Init ()						-- initializes lock for frame maanger
          newFramesAvailable = new Condition					-- condition variable for front thread waiting for resources
          newFramesAvailable.Init ()						-- initializes condition variable
	 GroupsWaiting= new Condition						-- Condition variable for the remaining threads waiting for resource threads except the leader
	GroupsWaiting.Init()							  -- initializes condition variable
...
-- This method provide a new frame ;and  it will make requesting threads to wait
-- until frames are available.
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
		...
		var i:int
		frameAddr,f:int
	frameManagerLock.Lock()									  -- Locks the monitors mutex for frame manager

		aPageTable.SetToThisPageTable()
		
		if numWaitingGroups > 1								  -- if there are any thread other then leader waiting,for frames make it wait 
			GroupsWaiting.Wait( &frameManagerLock )		
		endIf
		while(numberFreeFrames< numFramesNeeded )					-- if available frames are less then needed frames to fulfil request then
			newFramesAvailable.Wait( &frameManagerLock)				-- make leader wait
		endWhile
		...
		numberFreeFrames=numberFreeFrames-numFramesNeeded				-- update count for available frames after fulfilling request
		numWaitingGroups= numWaitingGroups - 1						-- now decrement count for threads waiting to acquire frames
		GroupsWaiting.Broadcast( &frameManagerLock)					-- Broadcast remaing threads if frames are availabe
...
-- This method will return all the frames once they are free 
-- after releasing the frames it will broadcast the remaiing threads waiting for frames
	...
	for i=0 to numFramesToReturn-1
		frameAddr=aPageTable.ExtractFrameAddr(i)					-- it will extract frame address for all frames that needs to be returned
		frameNumber= (frameAddr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME ) / PAGE_SIZE	-- calculates the framenumber
		framesInUse.ClearBit(frameNumber)
		numberFreeFrames=numberFreeFrames+1						-- after returning one frame make numFreeFrames count plus 1
	endFor
	newFramesAvailable.Broadcast(&frameManagerLock)						-- broadcast all the waiting thread in waiting list for leadr thread
	aPageTable.numberOfPages= 0								-- after returning all frames now page table has no entries
	frameManagerLock.Unlock()								 -- Unlocks the monitor mutex for frame Manager
...
-----------------------------  Handle_Sys_Exit  ---------------------------------
 ...
 -- This function will cause the current process and its thread to terminate
 -- The return status obtained from it will be pssed to Sys_Join of parent and the function will never return
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  -- This fnction will make the kernel to shutdown immediately
...
-----------------------------  Handle_Sys_Yield  ---------------------------------
  ...
  function Handle_Sys_Yield ()
   ...
   --This function will give/ yeild the cpu to the next first process on ready list
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  -- This process will create a clone of parent process with the copy of virtual memory space of parent
  -- both parent and child processes will return from this process    
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
   ...
   -- This function causes the caller to wait until the process with the given pid has terminated, by executing a call to Sys_Exit.	
   -- ReturnStatus will be returned from this function
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
 ...
 --It is read in to memory, overwriting the entire address space of the current process. 
 -- Then the OS will begin executing the new process.
 -- This function will not return
    ...
    var 
      ...
      -- Step 3: Create the new logical address space for the executable file
      ...
      -- Step 4: Close the executable file
      
      fileManager.Close (pOpenFile)

      -- Step 5: Compute the user-level stack value, initUserStackTop.
      
      initUserStackTop = (currentThread.myProcess.addrSpace.numberOfPages * PAGE_SIZE)

      -- Step 6: makes system stack top to full size
      
      initSystemStackTop = & currentThread.systemStack[SYSTEM_STACK_SIZE-1]
      oldStatus = SetInterruptsTo (DISABLED)
      currentThread.isUserThread = true

      --step 7: Make this thread a user thread so it can execute the executable file.
...
-----------------------------  Handle_Sys_Close  ---------------------------------
  function Handle_Sys_Close (fileDesc: int)
   ...
   --It closes the file, which includes writing out any data buffered by the kernel.
